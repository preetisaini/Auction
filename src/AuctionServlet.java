...
public class AuctionServlet extends HttpServlet 
{
	...
	public void doGet(HttpServletRequest req,HttpServletResponse res) throws ServletException,IOException
	{
		...
		if(rs.next())
		{
			...
			else if(amt>=(hbid+min))
			{
               ...
               // ResultSet rst=stmt.executeQuery("select enddate-curdate() from item_master where itemid="+id); 
				 ResultSet rst=stmt.executeQuery("select time_to_sec(timediff(time(sysdate()),time(stdate))) from item_master where itemid="+id);
				if(rst.next())
				{
					int dif=rst.getInt(1);
					if(dif>300)
					{
						 pw.println("<center><b>auction is closed a "+dif+" seconds(s) before.To view auction result,click on link 'View Auction Results'.</b><br<br></center>");
					}
				/*if((year >= year1))
				{
					if((month >= month1))
		...
		else
		{
			...
			{
				pw.println("<br><center>Please Login/Register to use this service</center>");
			}			
		...
		pw.println("<STRONG></STRONG><A href=./Results.jsp>View Auction Results</A>");
