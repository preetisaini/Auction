   ...
   <TR>
    <TD>
     <A href="./home.jsp"><STRONG>
          <FONT COLOR="BROWN">HOME</STRONG>
     </A> </TD>    
    
        <TD>
    <A href="./registration.jsp"><STRONG>
        <FONT COLOR="BROWN">REGISTER</STRONG>
     </A></TD>
        <TD>
      <A href="./sell.jsp"><STRONG>
        <FONT COLOR="BROWN">SELL</STRONG>
       </A></TD>
	   <TD>
      <A href="./ForSale.jsp"><STRONG>
        <FONT COLOR="BROWN">CURRENT BID ITEMS</STRONG>
       </A></TD>
        <TD>
     <A href="./help.jsp"><STRONG>
        <FONT COLOR="BROWN">HELP</STRONG>
    </A></TD>
...
<center> 
      <%
	    ...
	    ResultSet rs2=st.executeQuery("select b.bidderid, b.bamt,b.biddate,b.itemid from bidding_info b inner join (select  min(biddate)as mindate,itemid,bamt from bidding_info group by itemid,bamt)bm on b.bamt=bm.bamt and b.biddate=bm.mindate and b.itemid=bm.itemid inner join (select itemid from item_master where time_to_sec(timediff(time(sysdate()),time(stdate)))<300) ibm on  b.itemid=ibm.itemid where b.bamt=(select max(bamt) from bidding_info ) group by itemid");
	    	    
	    	    
      //System.out.println("resultset has values");
//ResultSet rs2=st.executeQuery		
		out.println("<center><table border=3 bordercolor=green>");
		out.println("<thead><th>Item ID</th><Th>Bid Amount</th><Th>Winner ID </th></thead>");
		//out.println("<Th>Seller ID</th><Th>Bidder ID</th><Th>Bid Amount</th></thead>");
		while(rs2.next())
		{
		String bidderid=rs2.getString(1);
		String bamt=rs2.getString(2);
		//String catid=rs2.getString(2);
		//String itemname=rs2.getString(3);
        //String sprice=rs2.getString(4); 
		java.sql.Date edate=rs2.getDate(3);
		String itemno=rs2.getString(4);
